...
class UserSubscriptionService {
  constructor(
    @inject('IUserSubscriptionRepository') private userSubscriptionRepository: IUserSubscriptionRepository,
  ...
  async create(payload: CreateUserSubscriptionDTO): Promise<UserSubscription> {
    const subscription = await this.subscriptionService.findById(payload.subscriptionId);
    if (!subscription) {
